...
if ($_smode < 2) // user mode
{
	...
	if ($_cfg['Const_IntCurr'])
		setPage('curr1', $_currs[1]);
	elseif (count($_currs) == 1)
		setPage('curr1', reset($_currs));

	// Main vars
		
	$_GS['lang'] = getLang($_SESSION['_lang']); // lang
	$_GS['lang_dir'] = getLangDir(); // tpl lang dir
...
{
	$_currs = $db->fetchIDRows($db->select('Currs',
		'*', 'cDisabled=0', null, 'cID'), false, 'cID');
}
