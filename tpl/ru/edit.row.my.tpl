...
{else}							{*** PREPARE (FOR ALL) ***}
	...
	{if !$v.readonly and ($v[0] != 'X')}
		{$l="<label class='col-sm-4 control-label text-right' for=\"{$edit_form_name}_$f\">$l</label>"}
	...
	{if $v[0] == 'CC' or $v[0] == 'RR' or $v[0] == 'Y' or $v[0] == 'W' or $v[0] == 'XX' or $v[0] == 'L'}
		<td colspan="2" align="center">
			{if $v[0] == 'CC'}
				{$error_text}
				{$l}
				<div class="col-sm-6"><input{$n}value="1"{if $cv} checked="checked"{/if} type="checkbox" class="checkbox{$class}{$error_class}"></div>
			{elseif $v[0] == 'RR'}
				{$error_text}
			...
			{else}
				...
				{elseif $v[0] == 'W'}
					<div class="col-sm-6"><textarea{$n}{$hint}cols="60"{if $v.size > 0} rows="{$v.size}"{/if} wrap="off" class="form-control text{$class}{$error_class}">{$cv}</textarea></div>
				{elseif $v[0] == 'XX'}
	...
	{else}
		...
		<div class="col-sm-6">
			...
			{elseif $v[0] == 'S'}
				<select{$n}{if $v.size > 0} size="{$v.size}"{/if}{if $v.multiple} multiple="multiple"{/if} class="form-control select{$class}{$error_class}">
			...
			{elseif $v[0] == 'I'}
				<input{$n}value="{$cv}" size="{if $v.size > 0}{$v.size}{else}4{/if}"type="text" class="form-control integer{$class}{$error_class}">
			{elseif in_array($v[0], array('F', '$', '%'))}
				<input{$n}value="{$cv}" size="{if $v.size > 0}{$v.size}{else}10{/if}"type="text" class="form-control float{$class}{$error_class}">
			{elseif $v[0] == '*'}
				<input{$n}{$hint}value="{_IN($f)}" size="{if $v.size > 0}{$v.size}{else}20{/if}"type="password" class="form-control password{$class}{$error_class}">
			{else}
				<input{$n}{$hint}value="{$cv}" size="{if $v.size > 0}{$v.size}{else}20{/if}"type="text" class="form-control string_small{$class}{$error_class}">
			{/if}
		{/if}
								{*** COMMENTS ***}
			{if $v.readonly}
