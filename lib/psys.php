...
function getPayFields($cid) { // public payin info
	...
	case 'PY': 
		return array(
			'acc' => 	array('Account number', 'P\d{5,8}', 'P1234567')
		);
	case 'NXR': 
		return array(
...
//			memo - from user
	...
	case 'YM':
		$r = array(
			'accfrom' => $arr['sender'],
			'sum' => round($arr['amount'] / 0.995, 1),
			'sum2' => round($arr['amount'] / 0.995, 1),
			'curr' => $arr['currency'],
			'tag' => $arr['label'],
			'date' => time(),
...
//			'batch' => $arr['transaction_hash'],
	...
	case 'IBC':
		...
		$r = array(
			'sum' => $res['data']['available_balance'],
			'curr' => $arr['data']['network'],
			'tag' => _GETN('tag'),
			'date' => time(),
			'batch' => $arr['notification_id'],
			'hash' => _GET('key')
		);
		$r['cc'] = $params2['key'];
...
*/
	...
	case 'AC':
		...
		try {
			...
			if ($res['answer'] = strval($getBalancesResponse->return))
...
//
	...
	case 'IBC':
		...
		{
			$res['result'] = 'OK';
			$res['batch'] = md5(time() . $toparams['acc'] . $sum);
		}
		else
			$res['result'] = $answ['data']['error_message'];
		return $res;
